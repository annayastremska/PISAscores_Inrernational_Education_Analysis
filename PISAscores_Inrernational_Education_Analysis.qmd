...
body { font-family: 'Bricolage Grotesque', sans-serif; color: #2E3250; line-height: 1.7; }
h1, h2, h3 { color: #1A1E3A; font-weight: 700; }
h4 { font-weight: 400; font-size: 0.92rem; color: #8A8FA8; letter-spacing: 0.06em; text-transform: uppercase; margin-bottom: 0.3rem; }
.callout-note  { border-left-color: #1170AA; }
.callout-tip   { border-left-color: #57A44C; }
.callout-warning   { border-left-color: #FC7D0B; }
.callout-important { border-left-color: #C85200; }
.lead { font-size: 1.05rem; color: #4A4E6A; font-weight: 300; }
.insight-box { background: #EEF0F8; border-left: 4px solid #1170AA; padding: 1rem 1.25rem; border-radius: 0 6px 6px 0; margin: 1.5rem 0; font-size: 0.95rem; }
.fig-caption { text-align: center; font-size: 0.82rem; color: #8A8FA8; font-style: italic; margin-top: 0.2rem; margin-bottom: 1rem; }
.column-screen-inset img, .quarto-figure img, figure img, p > img { width: 860px; max-width: 100%; height: auto; display: block; margin: 0 auto; }
.column-screen-inset { margin-top: 0.5rem; margin-bottom: 0; padding: 0; text-align: center; }
.fig-caption + hr, .fig-caption + p, .fig-caption + h2, .fig-caption + h3, .fig-caption + h4 { margin-top: 0.75rem; }
hr { margin: 1.75rem 0; }
details.code-fold { margin-bottom: 0.5rem; }
#TOC a, #TOC li > a, #TOC li > a strong, #TOC li > a b { font-weight: 400 !important; white-space: normal; word-break: break-word; }
#TOC .active > a { font-weight: 600 !important; }
#TOC ul ul { display: none !important; }
#quarto-margin-sidebar { overflow-x: hidden; }
.subquestion { font-size: 1.08rem; color: #2E3250; font-style: italic; font-weight: 400; background: linear-gradient(90deg, #EEF0F8 0%, transparent 100%); border-left: 3px solid #1170AA; padding: 0.55rem 1rem 0.55rem 1rem; border-radius: 0 6px 6px 0; margin: 0 0 1.2rem 0; display: block; }
.report-hero-inner details.code-fold > summary, .report-hero-inner .code-fold-btn { background: rgba(255,255,255,0.15) !important; color: #ffffff !important; border-color: rgba(255,255,255,0.3) !important; border-radius: 4px; padding: 0.2rem 0.6rem; }
</style>

<script>
document.addEventListener("DOMContentLoaded", function () {
  // Inject "Data & Setup" into TOC if Quarto missed it (heading is inside raw HTML div)
  var toc = document.querySelector("#TOC > ul");
  if (toc && !document.querySelector('#TOC a[href="#data-setup"]')) {
    var li = document.createElement("li");
    li.className = "toc-item";
    var a = document.createElement("a");
    a.href = "#data-setup";
    a.textContent = "Data & Setup";
    li.appendChild(a);
    toc.insertBefore(li, toc.firstChild);
  }
  // Hide all nested TOC lists (show only top-level sections)
  document.querySelectorAll("#TOC ul ul").forEach(function(ul) {
...
::: lead
Every three years, the OECD tests half a million 15-year-olds across dozens of countries in math, reading, and science. The result is PISA — a snapshot of what the world's students can do. This report explores the data (2000 - 2018) from three main perspectives: **continental performance throughout time, geographical concentration & mobility of skills, and gender gap.**
:::
...
The analysis draws on three OECD PISA datasets (math, reading, science) merged with geographic metadata, international student enrollment figures from *Open Doors 2025*, and World Bank population and GDP estimates. Each row represents an individual student.
...
gdp_clean <- gdp_data |>
  select(LOCATION = iso3c, TIME = year, GDP = NY.GDP.PCAP.KD, income) |>
  filter(!is.na(GDP))

pisa_with_gdp <- pisa_final_l |>
  left_join(gdp_clean, by = c("LOCATION", "TIME")) |>
  mutate(
    avg_score = rowMeans(across(c(Math, Reading, Science)), na.rm = TRUE),
    continent = countrycode(LOCATION, origin = "iso3c", destination = "continent")
  )
```
...
`Oceania` in this dataset consists entirely of **Australia and New Zealand** — two wealthy, English-speaking OECD members. Less developed Pacific island nations do not participate in PISA. This is a structural bias: regions that look best are often those where the lower-performing countries simply aren't in the room. India, for instance, has never participated.
:::

### The Asia Spike

::: subquestion
Asia's dramatic jump in 2015 is not an educational miracle. It is a matter of who showed up.
:::
...
unique(pisa_wide$country[pisa_wide$continent == "Asia" &
                           pisa_wide$TIME == 2009]) |>
  knitr::kable(col.names = "Test Participants (Asia, 2009)")
```
...
unique(pisa_wide$country[pisa_wide$continent == "Asia" &
                           pisa_wide$TIME == 2015]) |>
  knitr::kable(col.names = "Test Participants (Asia, 2015)")
```
::: column-screen-inset
...
Singapore (avg: 564), Hong Kong (548), Macao (544), and Taiwan (542) — the top-scoring Asian economies — each participated in **only one round** — 2015. When those well developed areas are included, the continental average rises. When they drop out, it falls back.
...
The downward-sloping trend is clear: countries that started low improved; countries that started high declined or stagnated.
...
**Biggest decliners:** Finland, New Zealand, Australia, Iceland, the Netherlands — all wealthy, stable, long-established PISA participants. Whether this reflects genuine educational decline or regression to the mean is hard to establish without further economic analysis.
...
## III. Student Specialization & Mobility

### Does PISA Score Predict What Students Study?
...
ggplot(dat, aes(avg_score, students_per_million)) +
  geom_point(aes(colour = subject), size = 2.5, show.legend = F) +
  geom_smooth(method = "lm", se = T, colour = "grey30",
  ...
  geom_text(
    ...
    size = 3.2, colour = "#4A4E6A", fontface = "bold", inherit.aes = F
  ) +
...
This is not evidence that PISA scores *cause* field selection. Wealthier countries both score higher and send more students abroad — *GDP* is almost certainly the dominant factor. Moreover, this dataset only captures students studying in the **United States**; many European countries send students to the UK or France, which would suppress their numbers here.
:::

------------------------------------------------------------------------

### Does PISA Predict Total Enrollment Abroad?
...
::: fig-caption
fig. 3.2 Score VS Number of Students
:::

The correlation drops to **r = 0.4** — weaker than any of the subject-specific correlations. Hence, PISA explains *field choice* better than it explains *volume*.

**Conclusion:** The decision to study abroad at all — and specifically in the US — is dominated by factors outside educational attainment: GDP, English language proficiency, family wealth, visa regimes, cultural ties, and the availability of domestic alternatives.
...
### The Gender Gap in Pisa Subjects
...
**Math:** Boys lead almost everywhere. Interestingly, girls outperform boys in Northern European countries — where gender equality norms are strongest.

**Reading:** Girls lead in *every single country*, often by more than boys lead in math. Something holds regardless of cultural context: girls acquire verbal skills earlier and, allegedly, read more for pleasure. It is one of the most consistent findings in comparative education.

**Science:** Mixed. There is a slight average advantage for boys, but many gaps are close to zero, and the pattern is country-specific. There is no clean story here.

::: callout-tip
### The asymmetry that gets overlooked

Public discourse focuses heavily on women in STEM and rarely on men in humanities. Both gaps are real, and both deserve attention.
:::
...
|---|---|
| **I. Global Performance** | Oceania is the lead in all three subjects due to biased representation -- Australia and New Zealand. At the same time, the most powerful Asian economies barely participate in pisa, bringing the continental average down. |
| **II. Convergence** | Convergence is real but asymmetric --- low scorers improved modestly while top scorers declined sharply. Poland and Latvia are the standout reformers.|
| **III. Student Specialization & Mobility** | PISA scores predict *what* students study better than *how many* go abroad. The reading-to-humanities link is strongest; |
| **IV. Gender Gap** | Girls lead in reading in every single country; boys lead in math almost everywhere, but that gap shrinks in countries with high levels of gender equality. |
